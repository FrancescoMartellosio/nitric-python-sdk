...
class ResourceType(betterproto.Enum):
    ...
    KAFKA_SOURCE = 1
    """Translated producers / consumers"""

    KAFKA_SINK = 2
    KV_STORE_SOURCE = 3
    KV_STORE_SINK = 4
    HANDOFF_KAFKA_SOURCE = 5
    """Injected at partition boundaries"""

    HANDOFF_KAFKA_SINK = 6
    HANDOFF_KV_SOURCE = 7
    HANDOFF_KV_SINK = 8
...
class BatchHints(betterproto.Message):
    ...
    """
    Global fallback. Ignored when per_topic_starting_offsets has an entry for
    the topic.
    """

    per_topic_starting_offsets: Dict[str, str] = betterproto.map_field(
        5, betterproto.TYPE_STRING, betterproto.TYPE_STRING
    )
    """
    Per-topic starting offset. Value: "earliest" or Spark JSON map
    {"topicName":{"0":50,"1":60}}.
    """

    per_topic_ending_offsets: Dict[str, str] = betterproto.map_field(
        6, betterproto.TYPE_STRING, betterproto.TYPE_STRING
    )
    """
    Per-topic ending offset (pins the snapshot boundary). Value: "latest" or
    Spark JSON map {"topicName":{"0":100,"1":120}}. Absent = "latest".
    """
...
@dataclass(eq=False, repr=False)
class ResourceNode(betterproto.Message):
    id: str = betterproto.string_field(1)
    resource_type: "ResourceType" = betterproto.enum_field(2)
    resource_name: str = betterproto.string_field(3)
    schema: List["FieldDef"] = betterproto.message_field(4)


@dataclass(eq=False, repr=False)
class Subgraph(betterproto.Message):
    name: str = betterproto.string_field(1)
    resource_nodes: List["ResourceNode"] = betterproto.message_field(2)
    step_nodes: List["Step"] = betterproto.message_field(3)
